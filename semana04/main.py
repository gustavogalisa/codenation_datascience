...
def q1():
    normal = np.quantile(dataframe['normal'], [0.25,0.50,0.75])
    binomial = np.quantile(dataframe['binomial'], [0.25, 0.50, 0.75])
...
def q2():
    ...
    asn = ECDF(dataframe['normal'])(media+desvio) - ECDF(dataframe['normal'])(media-desvio)
    return round(float(asn), 3)
q2()
...
def q3():
    normais = dataframe['normal'].agg(['mean', 'var'])
    binomiais = dataframe['binomial'].agg(['mean', 'var'])
    asn = np.around(binomiais - normais,3)
    return tuple(asn)
